  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
            <div className="space-y-2">
              ...
              <p className="text-slate-600 text-lg">Track and manage all your transactions</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className={`relative px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 transform hover:scale-105 active:scale-95 ${
                  isCapturedOnly 
              ...
              <button 
                onClick={payNow} 
                className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/40 font-semibold text-sm transform hover:scale-105 active:scale-95"
              >
                <span className="flex items-center gap-2">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h3 className="font-bold text-slate-900 text-lg">Filter Transactions</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200 bg-white hover:border-slate-300 placeholder:text-slate-400"
                />
              </div>
              ...
              <div className="relative">
                <select 
                  ...
                  className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl appearance-none bg-white hover:border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200 font-medium text-slate-700 cursor-pointer"
                >
              ...
              <button
                ...
                className="px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-900 rounded-xl transition-all duration-200 font-semibold border-2 border-slate-200 hover:border-slate-300 active:scale-95"
              >
                Reset Filters
              </button>
            </div>
        ...
        {/* Error Message */}
        {error && (
          <div className="mb-6 p-4 bg-rose-50 border-2 border-rose-200 rounded-xl text-rose-700 flex items-center gap-3 shadow-md">
        ...
        {/* Loading State */}
        {loading && (
          <div className="flex flex-col items-center justify-center h-64 bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50">
            <div className="relative">
              <div className="animate-spin rounded-full h-16 w-16 border-4 border-slate-200"></div>
              <div className="animate-spin rounded-full h-16 w-16 border-4 border-t-blue-600 absolute inset-0"></div>
            </div>
        ...
        {!loading && filteredPayments.length > 0 && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 overflow-hidden">
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-slate-50 to-blue-50 border-b-2 border-slate-200">
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">Payment ID</th>
                  ...
                  {filteredPayments.map((payment, index) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <span className="font-mono text-sm text-slate-700 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 group-hover:border-blue-200 transition-colors">
                          {payment.id}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-slate-900 text-lg">{formatCurrency(payment.amount)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 border border-blue-200 capitalize shadow-sm">
                          {payment.method}
                        </span>
                      </td>
                      <td className="px-6 py-4">{getStatusBadge(payment.status)}</td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-slate-700 truncate max-w-xs font-medium">{payment.description}</p>
                        {payment.status === "failed" && payment.error_description && (
                          <p className="text-xs text-rose-600 mt-1.5 bg-rose-50 px-2 py-1 rounded border border-rose-100 inline-block">
            ...
            {/* Pagination */}
            <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t-2 border-slate-200 bg-gradient-to-r from-slate-50 to-blue-50 gap-4">
              <div className="text-sm font-semibold text-slate-700 bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm">
                {isFetchAll 
                  ? `Showing ${filteredPayments.length} payments (fetched all)` 
                  : `Page ${currentPage} • Showing ${filteredPayments.length} items`}
              </div>

              <div className="flex items-center gap-2">
                <button 
                  onClick={handlePrevPage} 
                  disabled={currentPage === 1 || isFetchAll} 
                  className="p-3 rounded-xl transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-white active:bg-slate-100 bg-white/80 border-2 border-slate-200 hover:border-blue-300 disabled:hover:border-slate-200 shadow-sm hover:shadow-md transform hover:scale-105 active:scale-95 disabled:transform-none" 
                ...
                <button 
                  ...
                  disabled={!hasNextPage || isFetchAll} 
                  className="p-3 rounded-xl transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-white active:bg-slate-100 bg-white/80 border-2 border-slate-200 hover:border-blue-300 disabled:hover:border-slate-200 shadow-sm hover:shadow-md transform hover:scale-105 active:scale-95 disabled:transform-none" 
        ...
        {/* Empty State */}
        {!loading && filteredPayments.length === 0 && (
          <div className="text-center py-16 bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50">
            <div className="inline-flex p-6 bg-gradient-to-br from-slate-100 to-blue-100 rounded-full mb-6">
            ...
            <button 
              ...
              className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/40 font-semibold transform hover:scale-105 active:scale-95"
            >
              Reset Filters
            </button>
          </div>
